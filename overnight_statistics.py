...
def get_all_overnight_stats(predictions):
    ...
    overnight_statistics = [[None for i in range(10)] for i in range(no_subjects)]
    ...
    for subject_id in tqdm(range(no_subjects)):
        ...
        overnight_statistics[subject_id][1] = SOL
        overnight_statistics[subject_id][2] = REMOL
        overnight_statistics[subject_id][3] = WASO
        overnight_statistics[subject_id][4] = RA
        overnight_statistics[subject_id][5] = NA
        overnight_statistics[subject_id][6] = TIS[:,1]
        overnight_statistics[subject_id][7] = TIS[:,2]
        overnight_statistics[subject_id][8] = TIS[:,3]
        overnight_statistics[subject_id][9] = TIS[:,4]
        
    return overnight_statistics

def get_overnight_stats_one_subject(predictions, subject_id):
    overnight_statistics = [None for i in range(10)]
    ...
    overnight_statistics[1] = SOL
    overnight_statistics[2] = REMOL
    overnight_statistics[3] = WASO
    overnight_statistics[4] = RA
    overnight_statistics[5] = NA
    overnight_statistics[6] = TIS[:,1]
    overnight_statistics[7] = TIS[:,2]
    overnight_statistics[8] = TIS[:,3]
    overnight_statistics[9] = TIS[:,4]
    ...
    for stat_id in range(no_stats):
        for method_id in range(no_methods):
            for subject_id in range(no_subjects):
                ...
                CDF_here = convert_2_cdf(samples_here)
                CDF_grth = convert_2_cdf(samples_grth)
                ...
                # Wasserstein metric
                overnight_statistics_results[1,stat_id, method_id, subject_id] = diff.sum()*0.5
